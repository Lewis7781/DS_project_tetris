...
void updatemap(gameboard board,block test){
    cout<<*test.shape[3].begin()<<endl;
    cout<<*(test.shape[3].begin()+1)<<endl;
    cout<<*(test.shape[3].begin()+2)<<endl;
    cout<<*(test.shape[3].begin()+3)<<endl;
...
int main(int argc, const char * argv[]) {
   ...
   // auto testcase=board.cube[0];
   /* updatemap(board, testcase);
    testcase=board.cube[2];
    updatemap(board, testcase);*/
    for(int i=0;i<board.cubecnt;i++){
        updatemap(board, board.cube[i]);
    }
    return 0;
}
